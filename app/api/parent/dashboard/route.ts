...
export async function GET() {
  ...
  const [{ data: school }, { data: grade }, { data: individual }, { data: warnings }] = await Promise.all([
    ...
    studentIds.length ? supabase.from("warning_entries").select("id,student_id,warning_date,entry_type,delta,parent_visible_reason,created_at,students(name,grade)").in("student_id", studentIds).is("teacher_note", null).order("created_at", { ascending: false }).limit(50) : Promise.resolve({ data: [] }),
  ] as any);
