...
2. 제출 즉시 **모든 교사·관리자**에게 웹 푸시 알림이 전송됩니다. 알림을 누르면 교사 화면의 `조퇴 신청` 탭으로 바로 이동합니다.
3. 승인·반려 절차는 없습니다. 모든 선생님이 내용을 열람하고 `확인` 버튼으로 확인 기록을 남길 수 있습니다.
4. 선생님이 `출석부에 조퇴 기록`을 누르면 해당 날짜의 출결이 조퇴로 기록됩니다. 잘못 눌렀으면 `기록 취소`로 되돌립니다.
5. 학부모는 출석부에 기록되기 전까지 신청을 취소할 수 있고, 취소되면 선생님들께 알림이 갑니다.
...
- 해당 날짜가 **이미 조퇴로 기록되어 있으면** 아무것도 쓰지 않습니다.
- 해당 날짜에 **다른 출결 상태(예: 결석)가 있으면** 그 값을 이전 상태로 남기고 조퇴로 바꿉니다.
- `기록 취소`는 조퇴 직전 상태로 되돌립니다. 다만 그 사이에 출석부를 직접 고쳤다면 건드리지 않고 그대로 둡니다.
