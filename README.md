...
6. **승인이 확정되면 해당 날짜의 출석부에 조퇴로 자동 기록됩니다.** 교사가 출석 관리에서 따로 입력할 필요가 없습니다.
7. 학부모는 승인 전까지 신청을 취소할 수 있습니다.

전체 상태(`status`)는 두 결재 칸에서 트리거로 계산되므로, 한쪽 결재가 남아 있는데 승인 완료로 표시되는 일은 없습니다.

### 출석부 자동 기록 규칙

기존 출결 기록과 같은 방식(`attendance_change_batches` + `attendance_entries`)으로 저장되므로, 출석 관리 화면과 출석 통계에 그대로 반영되고 이력도 남습니다. 결재한 선생님이 작성자로 기록됩니다.

- 해당 날짜가 **이미 조퇴로 기록되어 있으면** 아무것도 쓰지 않습니다. 같은 신청을 다시 승인해도 줄이 늘지 않습니다.
- 해당 날짜에 **다른 출결 상태(예: 결석)가 있으면** 그 값을 이전 상태로 남기고 조퇴로 바꿉니다.
- 승인 후 **반려로 뒤집으면** 조퇴 기록을 승인 직전 상태로 되돌립니다. 다만 그 사이에 선생님이 출석부를 직접 고쳤다면 건드리지 않고 그대로 둡니다.
- 출석부 기록에 실패하더라도 승인 자체는 취소되지 않습니다. 결재 화면에 실패 안내가 뜨니 출석 관리에서 직접 입력해 주세요.
- 승인 알림이 이미 학부모에게 가므로, 출석 자동 기록으로 별도의 출결 안내문을 추가로 보내지는 않습니다.
