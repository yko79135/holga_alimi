self.addEventListener('push', event => {
  ...
  event.waitUntil(self.registration.showNotification(data.title || '홀가 학부모 포털', { body: data.body || '새로운 학교 알림이 도착했습니다. 앱에서 확인해주세요.', icon: '/icons/icon-192.svg', badge: '/icons/icon-192.svg', data: { url: data.url || '/dashboard', noticeId: data.noticeId || null }, tag: data.noticeId || 'holga-notice' }));
});
